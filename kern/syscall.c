...
// uint32_t vm_start = (uint32_t)s;
// uint32_t vm_end = (uint32_t)ROUNDUP((s + len), PGSIZE);
//     while(vm_start < vm_end)
//     {
//         if(!(curenv->env_pgdir[PDX(vm_start)] & PTE_U))
//         {
//             cprintf("the curenv has no permission to read memory %x\n", curenv->env_pgdir[PDX(vm_start)]);
//             env_destroy(curenv);
//         }
//         vm_start += PGSIZE;
//     }
